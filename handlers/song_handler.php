...
if (keys_exist($_GET, array("songid", "albumid"))) {
	$song_worker = new SongWorker();
	$album_worker = new AlbumWorker();
	$sid = $_GET["songid"];
	$aid = $_GET["albumid"];
	if (!$song_worker->getSong($sid) || !$album_worker->getAlbum($aid)) {
		header("Location: index.php;");
		die();
	}
	
	// Remove the song if they have it, add it if they don't.
	$ownership = new OwnershipWorker(usrid());
	if ($ownership->ownsSong($sid, $aid))
		$ownership->removeSong($sid, $aid);
	else
		$ownership->addSong($sid, $aid);
